...
export const formatDateWithTime = (date: string | undefined) => {
  if (!date) return '';
  return formatDate(date) + ' às ' + new Date(date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false });
};
