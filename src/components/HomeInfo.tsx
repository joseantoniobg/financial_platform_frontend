...
import { formatDateWithTime, formatDate, formatCurrency, formatUrlParams, groupSum } from "@/lib/utils";
import { StTable } from "./st-table";
import PieChartHome, { COLORS } from "./pie-chart-home";
import { ArrowDownIcon, ArrowDownToDot, ArrowLeftIcon, ArrowRightIcon, ArrowUpFromDotIcon, ArrowUpIcon, LineChartIcon } from "lucide-react";
...
export function HomeInfo({ showWelcomeMessage, user }: HomeInfoProps) {
    ...
    const [filters, setFilters] = useState<{ userId?: string, initialDate?: string; finalDate?: string, selectedRole: number }>({ userId: !showWelcomeMessage ? user.sub : undefined, initialDate: new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString().split('T')[0], finalDate: new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0).toISOString().split('T')[0], selectedRole: user?.roles[0].id || 0 });
    ...
    const fetchYearData = useCallback(async () => {
        ...
        setLoading(true);
        try {
        const params = formatUrlParams({ userId: filters.userId, selectedRole: filters.selectedRole, initialDate: initialYearDate, finalDate: finalYearDate });
    ...
    return (<div className="space-y-6">
          {showWelcomeMessage && setFilters && <Card className='flex justify-between items-center p-4'>
