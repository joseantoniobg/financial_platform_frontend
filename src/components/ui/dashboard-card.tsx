...
export function DashBoardCard({ title, items, balance, textColor, children, total, chart }: { title: string; items: { label: string; value: number }[]; balance?: number; textColor?: string; children?: React.ReactNode; total?: number; chart?: React.ReactNode }) {
    return (<div className="bg-[hsl(var(--card))] rounded-lg shadow-md p-6 border border-gray-200 dark:border-gray-700">
              <div className="flex items-top justify-between">
                <p className="text-lg font-medium text-[hsl(var(--foreground))]">{title}</p>
                {children}
              </div>
              ...
              <div className="mt-4 space-y-2">
                {items.map((item, index) => (
                  <div key={index} className="flex justify-between">
                    ...
                    <span className="text-sm font-medium text-[hsl(var(--foreground))]">{formatCurrency(item.value)}</span>
                  </div>
                ))}
              </div>
              {total && <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                <div className="flex justify-between">
                  <span className="text-lg text-[hsl(var(--foreground))]">Total:</span>
                  <span className="text-lg font-medium text-[hsl(var(--foreground))]">{formatCurrency(total)}</span>
                </div>
              </div>}
            </div>);
}